"""
Pipeline Runner — CRA Decision Traceability System
"""
...
st.set_page_config(
    page_title="Pipeline — CRA System",
    page_icon="🔬",
    layout="wide",
...
with lc2:
    if st.button("🇯🇵 日本語",use_container_width=True,type="primary" if st.session_state.lang=="ja" else "secondary"):
        st.session_state.lang="ja"; st.rerun()

st.title("🔬 " + ("決定パイプライン" if st.session_state.lang=="ja" else "Decision Pipeline"))
...
with st.sidebar:
    ...
    selected_scenario=st.selectbox(t("sidebar_choose"),options=list(CVE_SCENARIOS.keys()),
                                   format_func=lambda k:t(f"scenario_{k}_name"),key="scenario_selector")
    ...
    prod=PRODUCTS[selected_product]; n=len(prod["sbom"]["components"])
    ...
    if run_btn:
        st.session_state.pipeline_phase="idle"; st.session_state.pre_review=None; st.session_state.pipeline_results=None
        if CVE_SCENARIOS[selected_scenario].get("human_review_required"):
            ...
            st.session_state.pipeline_phase="awaiting_human"; st.session_state.current_scenario=selected_scenario
        else:
            st.session_state.pipeline_results=run_pipeline(selected_scenario,selected_product)
            st.session_state.pipeline_phase="complete"; st.session_state.current_scenario=selected_scenario
    if st.session_state.runs_log:
        st.markdown("---"); st.header(t("sidebar_stats_header"))
        log=st.session_state.runs_log; decisions=[r["decision"] for r in log]
        ...
        with st.expander(t("sidebar_run_history")):
            for r in reversed(log): st.caption(f"`{r['ts']}` {r['scenario']} → **{r['decision']}**")

# ============= HUMAN REVIEW PANEL =============

if st.session_state.pipeline_phase=="awaiting_human" and st.session_state.pre_review:
    pre=st.session_state.pre_review; proposal=pre["decision_proposal"]
    ...
    st.dataframe(df.style.apply(lambda row:["background-color:#fff5f5" if t("t2_vulnerable") in str(row.iloc[-1]) else "" for _ in row],axis=1),use_container_width=True,hide_index=True)
...
if st.session_state.pipeline_results:
    ...
    with tab1:
        ...
        with col1: st.plotly_chart(cvss_gauge(results["cve"]["cvss_score"]),use_container_width=True)
        with col2:
            st.markdown(t("t1_description")); st.info(cve_desc(results["scenario_key"]))
            c1,c2,c3=st.columns(3)
            c1.metric(t("metric_cve"),results["cve"]["cve_id"]); c2.metric(t("metric_severity"),results["cve"]["severity"])
            exploit_label=("YES ⚠️" if results["cve"]["exploit_available"] else "NO ✅") if st.session_state.lang=="en" else ("あり ⚠️" if results["cve"]["exploit_available"] else "なし ✅")
            c3.metric(t("metric_exploit"),exploit_label)
    ...
    with tab2:
        ...
        col1.metric(t("metric_product"),match["product_name"]); col2.metric(t("metric_match_confidence"),f"{match['match_confidence']:.0%}")
        cf_label=("YES 🔴" if match["matching_component"] else "NO 🟢") if st.session_state.lang=="en" else ("あり 🔴" if match["matching_component"] else "なし 🟢")
        col3.metric(t("metric_component_found"),cf_label)
    ...
    with tab4:
        ...
        col3.metric(t("metric_auto_decidable"),t("t4_yes_auto") if decision["auto_decidable"] else t("t4_no_auto"))
        col_chart,col_rules=st.columns([1,1])
        with col_chart: st.plotly_chart(rule_confidence_chart(decision["rules_fired"]),use_container_width=True)
        with col_rules:
    ...
    with tab5:
        ...
        col1,col2,col3=st.columns(3)
        col1.metric(t("metric_reviewer"),review["reviewer"]); col2.metric(t("metric_action"),review["action"])
    ...
    with tab6:
        ...
        with col1:
            ...
            sub_label=("YES ✅" if enisa["submitted"] else "NO — Not required") if st.session_state.lang=="en" else ("あり ✅" if enisa["submitted"] else "なし — 不要")
            st.metric(t("metric_submitted"),sub_label)
        with col2:
            if enisa["submitted"]:
    ...
    with tab7:
        ...
        with col1:
            ...
            st.download_button(label=t("t7_html_btn"),data=html_report,file_name=f"CRA-Compliance-{results['cve']['cve_id']}.html",mime="text/html",use_container_width=True)
        with col2:
            st.markdown(t("t7_json_title")); st.caption(t("t7_json_caption"))
            st.download_button(label=t("t7_json_btn"),data=json.dumps(enisa_json,indent=2),file_name=f"ENISA-{results['cve']['cve_id']}.json",mime="application/json",use_container_width=True)
